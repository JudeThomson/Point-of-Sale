    ...
    <body>
        ...
        <script>
            ...
            async function fetchPurchase() {
                try {
                    const response = await fetch('/stock_report/data');
                    stock_transfer = await response.json();
                    renderTable(stock_transfer); 
                } catch (error) {
            ...
            document.getElementById('searchBtn').addEventListener('click', function() {
                ...
                const filteredPurchase = stock_transfer.filter(details => {
                const dateCondition = (!fromDate || !toDate) || (details.trans_date >= fromDate && details.trans_date <= toDate);
            ...
            function renderTable(data) {
                ...
                data.forEach(details => {
                    ...
                    row.innerHTML = `
                        <td>${details.trans_no}</td>
                        <td>${details.trans_date}</td>
                        <td>${details.from_ware}</td>
                        <td>${details.to_ware}</td>
                        <a href="/stock/print/${details.trans_no}">
